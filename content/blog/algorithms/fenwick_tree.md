...
```go
	Sum(i) = fsum(i) + fsum(g(x)) + fsum(g(g(x))) 
        + ... + fsum(g((....(x))))
```
...
```go
	...
	Sum(21) = [sum of elements from 21 to 21] 
        + [sum of elements from 17 to 20] 
        + [sum of elements from 1 to 16]
```
...
```go
    number of contributions by input[i] = 
        COUNT(fsum(i), fsum(h(x)), fsum(h(h(x))), ... )
```
